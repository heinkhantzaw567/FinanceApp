...
type TxType = 'charge' | 'refund' | 'transfer'
type TransferDirection = 'in' | 'out'
type BankId = number
type AccountType = 'credit' | 'chequing'
type AccountKey = 'td_credit' | 'td_chequing' | 'bmo_credit' | 'bmo_chequing' | 'other'
...
interface BankRoute {
  ...
  table: 'td_transactions' | 'bmo_transactions'
  banksTable: 'td_banks' | 'bmo_banks'
...
let tdDb: Database.Database
let bmoDb: Database.Database
...
function accountTypeForBankId(bankId: BankId): AccountType {
  return bankId === 2 || bankId === 4 ? 'chequing' : 'credit'
}

function accountKeyForBankId(bankId: BankId): AccountKey {
  switch (bankId) {
...
function bankRoute(bankId: BankId): BankRoute {
...
const TRANSFER_KEYWORDS: Record<AccountKey, RegExp[]> = {
...
function detectAndTagTransfer(
  row: { date: string; description: string; amount: number; type: TxType },
  route: BankRoute
): DetectResult {
  ...
  const intraMatch = db.prepare(`
    ...
    LIMIT 1
  `).get(row.date, row.amount, accountType) as { id: string; account_type: AccountType } | undefined

  if (intraMatch) {
    // Update the already-inserted counterpart
    const counterpartDirection: TransferDirection = intraMatch.account_type === 'credit' ? 'in' : 'out'
  ...
  if (accountType === 'chequing') {
    ...
    const crossMatch = otherDb.prepare(`
      ...
      LIMIT 1
    `).get(row.date, row.amount) as { id: string } | undefined

    if (crossMatch) {
      const isOutgoing = OUTGOING_CHEQUING_PATTERN.test(row.description)
      const currentDirection: TransferDirection    = isOutgoing ? 'out' : 'in'
      const counterpartDirection: TransferDirection = isOutgoing ? 'in'  : 'out'
...
function categorizeExpense(description: string): string {
  ...
  if (
    upper.includes('GROCERY') || upper.includes('GROCERIES') || upper.includes('METRO') ||
  ...
  if (
    upper.includes('SHOPPERS DRUG MART') || upper.includes('REXALL') ||
  ...
  if (
    upper.includes('AMAZON') || upper.includes('AMZN') || upper.includes('WINNERS') ||
  ...
  if (
    upper.includes('FREEDOM MOBILE') || upper.includes('ROGERS') ||
  ...
  if (
    upper.includes('NETFLIX') || upper.includes('SPOTIFY') || upper.includes('DISNEY') ||
...
function computeBalance(row: BankRow): number {
...
function queryBankStats(db: Database.Database, txTable: string, banksTable: string): BankRow[] {
  return db.prepare(`
    SELECT
...
function createSchema(db: Database.Database, txTable: string, banksTable: string): void {
...
function migrateFromLegacy(userDataPath: string): void {
  const oldDbPath = path.join(userDataPath, 'cashflow.db')
  if (!fs.existsSync(oldDbPath)) return

  const already = (
    tdDb.prepare(`SELECT value FROM settings WHERE key = 'legacy_migrated'`).get() as
  ...
  try {
    ...
    for (const row of oldDb.prepare(`SELECT id, opening_balance, credit_limit FROM banks`).all() as any[]) {
      const r = bankRoute(row.id as BankId)
    ...
    const txs = oldDb.prepare(`SELECT * FROM transactions`).all() as any[]
    ...
    const migrateTd = tdDb.transaction(() => {
      for (const tx of txs) {
        const r = bankRoute(tx.bank_id as BankId)
        if (r.db !== tdDb) continue
        const isPayment = tx.category === 'Credit Card Payment'
        insertTd.run(
          tx.id, tx.date, tx.description, tx.amount,
          isPayment ? 'transfer' : tx.type,
          isPayment ? (r.accountType === 'credit' ? 'in' : 'out') : null,
          r.accountType, tx.bank_id, tx.category, tx.source, tx.created_at
        )
      }
    })
...
function ensureDbs(): void {
  const userDataPath = app.getPath('userData')

  tdDb  = new Database(path.join(userDataPath, 'td.db'))
  bmoDb = new Database(path.join(userDataPath, 'bmo.db'))
...
function checkAndNotify(): void {
  ...
  const row = tdDb.prepare('SELECT value FROM settings WHERE key = ?').get('last_notified_month') as
    | { value: string } | undefined
  if (row?.value === current) return

  const { start, end } = monthBounds(current)
  const { count: tdCount }  = tdDb.prepare(
    'SELECT COUNT(*) AS count FROM td_transactions WHERE date BETWEEN ? AND ?'
  ).get(start, end) as { count: number }
  const { count: bmoCount } = bmoDb.prepare(
    'SELECT COUNT(*) AS count FROM bmo_transactions WHERE date BETWEEN ? AND ?'
  ).get(start, end) as { count: number }

  tdDb.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').run('last_notified_month', current)

  if (tdCount + bmoCount > 0) return
  ...
  notification.on('click', () => {
    if (mainWindow) { mainWindow.show(); mainWindow.focus(); mainWindow.webContents.send('open-import') }
  })
  notification.show()
}
...
function createWindow(): void {
  ...
  if (isDev) {
    ...
    mainWindow.loadFile(path.join(__dirname, '../dist/index.html')).catch((err) => console.error('[main] loadFile failed:', err))
  }
}
...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle('cashflow:get-dashboard', (_event, month: string) => {
    ...
    const monthlySeries = seriesMonths.map((m) => {
      ...
      `).get(b.start, b.end) as { charges: number; refunds: number }
  ...
  ipcMain.handle('cashflow:get-transactions', (_event, filters: TransactionFilter) => {
    ...
    if (typeof filters.bankId === 'number') { where.push('bank_id = ?'); params.push(filters.bankId) }
  ...
  ipcMain.handle('cashflow:get-spending-breakdown', (_event, month: string, bankId: number | null) => {
    ...
    function query(db: Database.Database, table: string, bankFilter: string, p: (string | number)[]): SpendingBreakdownRow[] {
      return db.prepare(`
      ...
      `).all(...p) as SpendingBreakdownRow[]
    }

    let rows: SpendingBreakdownRow[]
    ...
    if (bankId !== null) {
      ...
      const merged  = new Map<string, SpendingBreakdownRow>()
  ...
  ipcMain.handle('cashflow:add-manual-transaction', (_event, payload: TransactionInput) => {
    ...
    route.db.prepare(`
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?)
    `).run(
      randomUUID(), payload.date, payload.description, amount,
      detected.type, detected.transfer_direction, route.accountType,
      payload.bankId, payload.category, payload.source ?? 'manual', new Date().toISOString()
    )
    return true
  })
  ...
  ipcMain.handle('cashflow:get-existing-keys', (_event, candidates: DuplicateCandidate[]) => {
    ...
    const tdRows  = tdDb.prepare('SELECT date, description, amount, bank_id FROM td_transactions').all()  as any[]
    const bmoRows = bmoDb.prepare('SELECT date, description, amount, bank_id FROM bmo_transactions').all() as any[]

    const existingSet = new Set(
      [...tdRows, ...bmoRows].map(
        (r) => `${r.date}|${r.description.trim().toUpperCase()}|${r.amount.toFixed(2)}|${r.bank_id}`
      )
    )
  ...
  ipcMain.handle('cashflow:import-transactions', (_event, payload: TransactionInput[]) => {
    ...
    for (const row of payload) {
      ...
      route.db.prepare(`
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?)
      `).run(
        randomUUID(), row.date, row.description, amount,
        detected.type, detected.transfer_direction, route.accountType,
        row.bankId, row.category, row.source ?? 'import', new Date().toISOString()
      )
      imported++
    }
  ...
  ipcMain.handle('cashflow:recategorize-all', () => {
    let updated = 0
    ...
    for (const { db, table } of [
      ...
      const rows = db.prepare(
        `SELECT id, description FROM ${table} WHERE type = 'charge'`
